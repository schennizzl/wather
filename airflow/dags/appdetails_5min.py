...
# Каждые 5 минут запрашиваем типы через API_URL_APPDETAILS (appdetails)
with DAG(
    dag_id="steam_appdetails_every_5m",
    ...
    fetch_types = BashOperator(
        task_id="fetch_types",
        env={"HTTP_PROXY": "", "HTTPS_PROXY": "", "ALL_PROXY": ""},
        bash_command=(
            f"python {FETCH_SCRIPT} --sort --api-key {STEAM_API_KEY} "
            f"-o {DATA_DIR}/steam_app_list_latest.json "
            f"--types-output {DATA_DIR}/{FILE_NAME} "
            f"--types-ndjson --types-checkpoint {DATA_DIR}/types_checkpoint.json "
            f"--types-limit {TYPES_BATCH_LIMIT} --types-sleep 0.05 "
