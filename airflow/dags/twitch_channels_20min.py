...
FETCH_SCRIPT = f"{PROJECT_DIR}/scripts/fetch_twitch_channels.py"
GAMES_FILE = f"{PROJECT_DIR}/games.txt"
DBT_PROJECT_DIR = f"{PROJECT_DIR}/dbt_steam"
S3_BUCKET = "raw"
S3_PREFIX = "steam/raw/twitch_channels/dt={{ ds }}/hour={{ execution_date.strftime('%H') }}"
FILE_NAME = "twitch_channels_{{ ts_nodash }}_{{ run_id | replace(':', '_') | replace('+', '_') }}_try{{ ti.try_number }}.ndjson"
...
def sync_landing_twitch_channels_partitions() -> None:
    conn = _trino_connect()
    cur = conn.cursor()
    cur.execute("CALL system.sync_partition_metadata('raw', 'twitch_channels_files', 'ADD')")
    cur.fetchall()
    conn.close()
...
with DAG(
    dag_id="steam_twitch_channels_every_20m",
    ...
    fetch_twitch_channels = BashOperator(
        task_id="fetch_twitch_channels",
    ...
    sync_landing_partitions = PythonOperator(
        ...
        python_callable=sync_landing_twitch_channels_partitions,
    )
    ...
    load_stg_trino = BashOperator(
        ...
        bash_command=(
            ...
            "dbt run --profiles-dir . --select stg_steam_twitch_channels"
        ),
    )
    ...
    test_dbt_models = BashOperator(
        ...
        bash_command=(
            ...
            "--select stg_steam_twitch_channels"
        ),
    )

    ensure_dirs >> fetch_twitch_channels >> upload_minio >> sync_landing_partitions >> load_stg_trino >> test_dbt_models
