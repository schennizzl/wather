...
FETCH_SCRIPT = f"{PROJECT_DIR}/scripts/fetch_twitch_viewers.py"
GAMES_FILE = f"{PROJECT_DIR}/games.txt"
DBT_PROJECT_DIR = f"{PROJECT_DIR}/dbt_steam"
S3_BUCKET = "raw"
S3_PREFIX = "steam/raw/twitch_viewers/dt={{ ds }}/hour={{ execution_date.strftime('%H') }}"
FILE_NAME = "twitch_viewers_{{ ts_nodash }}.ndjson"
...
def sync_landing_twitch_viewers_partitions() -> None:
    conn = _trino_connect()
    cur = conn.cursor()
    cur.execute("CALL system.sync_partition_metadata('raw', 'twitch_viewers_files', 'ADD')")
    cur.fetchall()
    conn.close()
...
with DAG(
    dag_id="steam_twitch_viewers_every_20m",
    ...
    fetch_twitch_viewers = BashOperator(
        task_id="fetch_twitch_viewers",
    ...
    sync_landing_partitions = PythonOperator(
        ...
        python_callable=sync_landing_twitch_viewers_partitions,
    )
    ...
    load_stg_trino = BashOperator(
        ...
        bash_command=(
            ...
            "dbt run --profiles-dir . --select stg_steam_twitch_viewers"
        ),
    )
    ...
    test_dbt_models = BashOperator(
        ...
        bash_command=(
            ...
            "--select stg_steam_twitch_viewers"
        ),
    )

    ensure_dirs >> fetch_twitch_viewers >> upload_minio >> sync_landing_partitions >> load_stg_trino >> test_dbt_models
